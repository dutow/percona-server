...
static int auth_ldap_simple_deinit(MYSQL_PLUGIN plugin_info
  ...
  // TODO: see possible crash mentioned in ldap_common
...
int mpaldap_simple_authenticate(MYSQL_PLUGIN_VIO *vio,
  ...
  // TODO: unlikely, but possible crash
  // * Enable this sleep & build with ASAN
  // * start the server
  // * start a mysql client: install the plugin, create a user
  // * on a different terminal: try to connect with this user
  // * on the first client, while the other sleeps: uninstall the plugin
  // * wait until the sleep ends
  //sleep(100);
